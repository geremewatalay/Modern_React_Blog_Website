...
const BlogCards = ({blogs, currentPage, selectedCategory, pageSize}) => {
  ...
  return (
    <div className='grid md:grid-cols-3 sm:grid-clos-2 grid-cols-1 gap-8'>
        {
            filteredBlogs.map((blog) => <Link to={`/blogs/${blog.id}`} key={blog.id} className='p-5 shadow-lg rounded cursor-pointer'>
                ...
                <p className='md-2 text-gray-400'><FaUser className='inline-flex items-center mr-2'/>{blog.author}</p>
